    ...
    for lr in 1e-5 1e-4 1e-3; do
      for var in $virtual_tokens_list; do
        python run.py \
          ...
          --run_name $TASK_NAME-$DATASET_NAME-$MODEL_NAME-$lr-$PEFT_TYPE-$k_shot-shot \
          --task_name $TASK_NAME \
          ...
          --output_dir checkpoints/PEFT/$PEFT_TYPE/$MODEL_NAME/$TASK_NAME-$DATASET_NAME-$lr-$k_shot-shot/ \
