...
class RPromptEmbedding(BaseEmbedding):
    """
    The model to encode virtual tokens into prompt embeddings that can prune the negative tokens and reduce dimension
    
    Args:
        config ([`RPromptTuningConfig`]): The configuration of the prompt embedding.
    ...
    **Attributes**:
    ...
    ```py
    >>> from peft import RPromptEmbedding, RPromptTuningConfig

    >>> config = RPromptTuningConfig(
    ...     peft_type="RPROMPT_TUNING",
    ...
    >>> rprompt_embedding = RPromptEmbedding(config, t5_model.shared)
    ```
    ...
    def __init__(self, config, word_embeddings):
        ...
        if config.inference_mode:
            self.reduced_embedding = torch.nn.Embedding(int(self.total_virtual_tokens * round(1 - self.config.token_ratio, 5)), self.token_dim)
    ...
    def forward(self, indices):
        # Just get embeddings
        prompt_embeddings = self.embedding(indices)[:, sorted(self.kept_prune[self.token_prefix]), :] * self.piece_mask.to(indices.device)
    ...
    def batch_piece_importance(self):
        ...
        split_embed = torch.split(embed[sorted(self.kept_prune[self.token_prefix])], split_size, dim=1)
        split_d_embed = torch.split(embed.grad[sorted(self.kept_prune[self.token_prefix])], split_size, dim=1)
    ...
    def estimate_token_importance(self, trainer, current_step):
        ...
        self.token_mask = self.token_mask[
            sorted(self.kept_prune[self.token_prefix])
        ]
        self.piece_mask = self.piece_mask[
            sorted(self.kept_prune[self.token_prefix])
        ]
        self.kept_prune[self.piece_prefix] = {
            f"{self.token_prefix}:{token}": set(range(self.config.token_pieces)) 
            for token in range(target_token_length)
        }
    ...
    def estimate_piece_importance(self, trainer, current_step):
        ...
        embed_importance = torch.zeros(len(self.kept_prune[self.token_prefix]), self.config.token_pieces).to(device)
        denoms = embed_importance.clone()
